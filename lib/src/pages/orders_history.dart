...
class OrdersHistoryWidget extends StatefulWidget {
  ...
  OrdersHistoryWidget({Key? key, required this.parentScaffoldKey}) : super(key: key);

  @override
  _OrdersHistoryWidgetState createState() => _OrdersHistoryWidgetState();
}

class _OrdersHistoryWidgetState extends AppStateX<OrdersHistoryWidget> {
  OrderController? _con;

  _OrdersHistoryWidgetState() : super(controller: OrderController()) {
    _con = controller as OrderController?;
  }

  @override
  void initState() {
    _con!.listenForOrdersHistory(message: '');
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        centerTitle: true,
        title: Text(S.of(context).orders_history, style: Theme.of(context).textTheme.headlineSmall?.merge(TextStyle(letterSpacing: 1.3))),
